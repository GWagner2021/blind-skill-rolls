...
export function resolveAbilityCheckVisibility(abilityId: string | null | undefined): VisibilityResult {
  const normalized = normalizeAbilityId(abilityId);
  if (!normalized) return { mode: null, force: false };
  if (isAbilityCheckBlind(normalized))   return { mode: BLIND,  force: true };
  if (isAbilityCheckPrivate(normalized)) return { mode: GMROLL, force: true };
  return { mode: null, force: false };
}

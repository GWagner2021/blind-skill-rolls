...
async function updateMidiConfig(cfgSettings: MidiConfigSettings): Promise<void> {
  if (!isMidiActive()) return;
  try { if (game.user!.isGM) await game.settings.set(MIDI_ID, "ConfigSettings", cfgSettings); }
  catch (e) { dbgWarn("midi-sync | failed to update midi-qol settings:", e); }
}

function buildBlindSkillList(): string[] {
  const areSkillsEnabled = (() => { try { return !!game.settings.get(MOD, "enabled"); } catch { return false; } })();
  if (!areSkillsEnabled) return [];
  const list: string[] = [];
...
function buildBlindSaveList(): string[] {
  const areSavesEnabled = (() => { try { return !!game.settings.get(MOD, "savesEnabled"); } catch { return false; } })();
  if (!areSavesEnabled) return [];
  const list: string[] = [];
  for (const ab of SAVE_LIST) {
    try { if (game.settings.get(MOD, "save_" + ab)) list.push(ab); } catch { /* ignore */ }
  }
  return list;
}

async function performInitialSync(): Promise<void> {
  if (!isMidiActive()) { dbgInfo("midi-sync | midi-qol not active – sync disabled"); return; }
  try {
    _isSyncing = true;
    _lastSyncDirection = "initial";

    let cfgSettings: MidiConfigSettings;
    try { cfgSettings = game.settings.get(MIDI_ID, "ConfigSettings") as MidiConfigSettings; }
...
async function syncBSRToMidi(): Promise<void> {
  if (!isMidiActive() || _isSyncing) return;
  if (_lastSyncDirection === "midi-to-bsr") { dbgWarn("midi-sync | Skipping BSR→midi (just synced FROM midi)"); _lastSyncDirection = null; return; }
  try {
    _isSyncing = true;
...
Hooks.on("updateSetting", async (document: SettingDocument) => {
  if (_isSyncing) return;

  if (document.key === `${MIDI_ID}.ConfigSettings`) {
    if (!isMidiActive()) return;
    if (_lastSyncDirection === "bsr-to-midi") { _lastSyncDirection = null; return; }
    try {
      _isSyncing = true;
      ...
      let midiSkills: string[] = cfgSettings.rollSkillsBlind || [];
      if (midiSkills.includes("all")) midiSkills = [...SKILL_LIST];
      let skillChanges = 0;
      for (const sk of SKILL_LIST) {

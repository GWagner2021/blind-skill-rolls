...
import {
  FF_SETTING_KEYS,
  ALL_BSR_FF_TYPES,
  getAutoFastForwardGM,
  getAutoFastForwardPlayer,
  setAutoFastForward,
  setAutoFastForwardPlayer,
  bsrToMidiArray,
  midiToBsrSet
} from "./ff-api.js";
...
let _ffSyncDirection: SyncDirection = null;
...
function arraysEqual(a: unknown, b: unknown): boolean {
  ...
  const s1 = [...a].sort();
  const s2 = [...b].sort();
  return s1.every((v: string, i: number) => v === s2[i]);
}

interface MidiConfigSettings {
  gmAutoFastForward?: string[];
  autoFastForward?: string[];
  [key: string]: unknown;
}

// ---- Initial: BSR ist Source of Truth ----
async function syncFastForwardInitial(): Promise<void> {
  ...
  try {
    ...
    if (!arraysEqual(cfgSettings.gmAutoFastForward, wantGm)) { cfgSettings.gmAutoFastForward = wantGm; changed = true; }
    if (!arraysEqual(cfgSettings.autoFastForward, wantPlayer)) { cfgSettings.autoFastForward = wantPlayer; changed = true; }
...
async function syncBSRToMidi(): Promise<void> {
  ...
  if (_ffSyncDirection === "midi-to-bsr") { _ffSyncDirection = null; return; }
  try {
    _ffSyncing = true;
...
// ---- MidiQOL → BSR ----
async function syncMidiToBSR(): Promise<void> {
  if (!isMidiActive() || _ffSyncing) return;
  if (_ffSyncDirection === "bsr-to-midi") { _ffSyncDirection = null; return; }
  try {
    _ffSyncing = true;
...
Hooks.on("updateSetting", (document: SettingDocument) => {

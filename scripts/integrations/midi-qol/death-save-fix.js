...
function handleDeathSavePreRoll() {
    if (!isMidiActive())
        return;
...
Hooks.on("dnd5e.preRollSavingThrowV2", (cfg) => {
    try {
        if (!cfg)
            return;
...
function isDeathSaveMsg(msg, data) {
    const d5msg = msg?.flags?.dnd5e ?? {};
    const d5data = data?.flags?.dnd5e ?? {};
    const typeFrom = (src) => String(src?.roll?.type ?? src?.type ?? src?.rollType ?? "").toLowerCase();
    if (typeFrom(d5msg) === "death" || typeFrom(d5data) === "death")
        return true;
    const flavor = msg?.flavor ?? data?.flavor ?? "";
    if (/death\s*sav/i.test(flavor) || /todesrettung/i.test(flavor))
        return true;
    try {

...
const isValid = (v) => v === "0" || v === "1" || v === "2";
async function setDsnGhost(v, { silent = false } = {}) {
    if (!game.user?.isGM)
        return;
    if (!game.modules.get(DSN_MOD)?.active) {
        if (!silent)
            ui.notifications?.warn(L("BSR.DSN.Error.NotActive"));
        return;
    }
    v = String(v);
    if (!isValid(v))
...
Hooks.once("init", () => {
    try {
        game.settings.register(MOD, PROXY_KEY, {
            ...
            default: "2",
            onChange: (v) => { if (game.user?.isGM)
                setDsnGhost(String(v)).catch(() => { }); }
        });

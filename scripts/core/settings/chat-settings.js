...
Hooks.once("init", () => {
    ...
    class BSRMenuChatDisplay extends HandlebarsApplicationMixin(ApplicationV2) {
        ...
        async _prepareContext(options) {
            ...
            return {
                ...context,
                hide: !!getSetting("hideForeignSecrets", true),
                mute: !!getSetting("muteForeignSecretSounds", true),
                sanitize: !!getSetting("bsrSanitizePublicGm", true),
                trusted: !!getSetting("bsrTrustedSeeDetails", false),
                maskDef: !!getSetting("bsrNpcMaskDefault", false),
                repl: getSetting("bsrNpcNameReplacement", ""),
                labels: {
    ...
    const COLOR_DEFAULTS = { bsrBlindCardColor: DEFAULT_BLIND_CARD_COLOR, bsrPrivateCardColor: DEFAULT_PRIVATE_CARD_COLOR };
    const GM_ONLY_KEYS = ["bsrCustomCardColorsEnabled", "bsrBlindCardColor", "bsrPrivateCardColor"];
    Hooks.on("renderSettingsConfig", (_app, el) => {
        if (game.user?.isGM) {
        ...
        for (const [key, def] of Object.entries(COLOR_DEFAULTS)) {
            const textInput = el.querySelector(`input[name="${MOD}.${key}"]`);
            if (!textInput)
                continue;
    ...
    function _checkbox(name, labelText, checked, disabled) {
    ...
    function injectChatDisplayControls(_app, html) {
        try {
            if (!(html instanceof HTMLElement))
                return;
            const form = html.querySelector("form") ?? html.querySelector(".window-content form") ?? html;
            if (!form)
                return;
            if (form.querySelector(".bsr-chatdisplay"))
                return;
            ...
            fs.append(_legend(L("BSR.Chat.Section.ChatDisplay", "Chat Display & Privacy")), _checkbox("bsrHide", L("BSR.Chat.Settings.HideForeign.Name", "Hide other players' secret messages"), getSetting("hideForeignSecrets", true), dis), _hint(L("BSR.Chat.Settings.HideForeign.Hint", "Hide blind rolls and whispers that are not addressed to the current player.")), _checkbox("bsrMute", L("BSR.Chat.Settings.MuteForeign.Name", "Mute other players' dice sounds"), getSetting("muteForeignSecretSounds", true), dis), _hint(L("BSR.Chat.Settings.MuteForeign.Hint", "Suppress dice sounds for secret rolls made by other players.")));
            // ── GM Rolls ──
            fs.append(_divider(), _legend(L("BSR.Chat.Section.GMRolls", "GM Roll Privacy")), _checkbox("bsrSanitize", L("BSR.Chat.Settings.GmSanitize.Name", "Strip roll details from public GM rolls"), getSetting("bsrSanitizePublicGm", true), dis), _hint(L("BSR.Chat.Settings.GmSanitize.Hint", "Remove dice formulas and tooltips from public GM rolls so players only see the result, not how it was calculated.")), _checkbox("bsrTrusted", L("BSR.Chat.Settings.TrustedDetails.Name", "Trusted players can see GM roll details"), getSetting("bsrTrustedSeeDetails", false), dis), _hint(L("BSR.Chat.Settings.TrustedDetails.Hint", "When enabled, players with the Trusted role or higher can still see dice formulas and tooltips on public GM rolls, even when stripping is active.")));
            // ── Chat Card Colors (client-scoped, all users) ──
            function _colorGroup(labelText, settingKey, fallback) {
                ...
                const stored = getSetting(settingKey, fallback);
            ...
            fs.append(_divider(), _legend(L("BSR.Chat.Section.CardColors", "Chat Card Colors")), _colorGroup(L("BSR.Chat.Settings.BlindCardColor.Name", "Blind GM Roll card color"), "bsrBlindCardColor", DEFAULT_BLIND_CARD_COLOR), _hint(L("BSR.Chat.Settings.BlindCardColor.Hint", "Background color for Blind GM Roll chat cards.")), _colorGroup(L("BSR.Chat.Settings.PrivateCardColor.Name", "Private GM Roll card color"), "bsrPrivateCardColor", DEFAULT_PRIVATE_CARD_COLOR), _hint(L("BSR.Chat.Settings.PrivateCardColor.Hint", "Background color for Private GM Roll chat cards.")));
            // ── NPC Masking ──
            ...
            replInput.value = getSetting("bsrNpcNameReplacement", "");
            replInput.className = "bsr-form-group__control";
            if (dis)
                replInput.disabled = true;
            replGroup.append(replLabel, replInput);
            fs.append(_divider(), _legend(L("BSR.NPC.Section.NameMasking", "NPC Name Masking")), _checkbox("bsrNpcMaskDefault", L("BSR.NPC.Settings.MaskDefault.Name", "Hide NPC names from players"), getSetting("bsrNpcMaskDefault", false), dis), _hint(L("BSR.NPC.Settings.MaskDefault.Hint", "When enabled, NPC names in chat and the combat tracker are hidden from players until explicitly revealed.")), replGroup, _hint(L("BSR.NPC.Settings.Replacement.Hint", "The name shown to players in place of hidden NPC names. Leave empty to use the default.")));
            form.appendChild(fs);
            // ── Bind change events (GM only) ──
            if (isGM) {
                const bind = (name, key) => {
                    fs.querySelector(`input[name="${name}"]`)?.addEventListener("change", (ev) => game.settings.set(MOD, key, ev.currentTarget.checked));
                };
                ...
                replInput.addEventListener("change", (ev) => game.settings.set(MOD, "bsrNpcNameReplacement", String(ev.currentTarget.value ?? "").trim()));
            }
        }
        catch (e) {
            dbgWarn("Inject Chat Display controls failed", e);
        }
    }

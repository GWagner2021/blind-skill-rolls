...
function isDeathSaveConfig(cfg) {
    try {
...
function handleDeathSavePreRoll(cfg, dialog, message, hookLabel) {
    try {
        ...
        if (vis.mode === BLIND) {
            ...
            guardedHookOnce("renderRollConfigurationDialog", (app, el) => setRollConfigSelectBlind(el ?? app?.element, DEATH_BLIND_NOTE_KEY, DEATH_BLIND_NOTE_FB));
        }
        else if (vis.mode === GMROLL) {
            setDsnPendingMode('private');
            guardedHookOnce("renderRollConfigurationDialog", (app, el) => setRollConfigSelectPrivate(el ?? app?.element, DEATH_PRIVATE_NOTE_KEY, DEATH_PRIVATE_NOTE_FB));
        }
    }
    catch (e) {
        dbgWarn(hookLabel, e);

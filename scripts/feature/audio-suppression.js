...
function isAddressedToMe(m) {
    ...
    if (!me)
        return false;
    const blind = !!m?.blind;
    const whisper = Array.isArray(m?.whisper) ? m.whisper : [];
    ...
    const push = (v) => {
        ...
        if (typeof v === "object" && v !== null) {
            ...
            if (typeof obj.src === "string")
                cand.push(obj.src);
            else if (Array.isArray(obj.src) && obj.src.length)
                cand.push(obj.src[0]);
...
export function openMute(ms = 3000, force = false) {
    ...
    if (!muteGate.active) {
        ...
        try {
            if (Howler) {
                muteGate.prevMuted = !!Howler._muted;
                Howler.mute(true);
            }
        }
        catch { /* ignore */ }
...
function extendMute(ms = 250) { if (!muteGate.active)
    return openMute(ms); muteGate.until = Math.max(muteGate.until, now() + ms); scheduleMuteCheck(); }
function scheduleMuteCheck() { if (!muteGate.timer)
    muteGate.timer = setTimeout(checkMute, 100); }
function checkMute() {
    muteGate.timer = null;
    if (!muteGate.active)
        return;
    if (now() <= muteGate.until)
        return scheduleMuteCheck();
    try {
        if (Howler && muteGate.prevMuted !== null)
            Howler.mute(!!muteGate.prevMuted);
    }
    catch { /* ignore */ }
    muteGate.active = false;
    muteGate.prevMuted = null;
    muteGate.until = 0;
    muteGate.force = false;
}
// ---- Patches ----
...
function patchAudioHelperPlay() {
    ...
    try {
        ...
        AH.play._bsrPatched = true;
        PATCHED.audio = true;
    }
    catch (e) {
        dbgWarn("audio-suppression | AudioHelper patch failed", e);
    }
}
function patchFoundrySoundClass() {
    if (PATCHED.sound)
        return;
    try {
        if (!foundry?.audio?.Sound?.prototype?.play)
            return;
        const _play = foundry.audio.Sound.prototype.play;
        if (_play._bsrPatched) {
            PATCHED.sound = true;
            return;
        }
        ...
        dbgWarn("audio-suppression | Sound.prototype.play patch failed", e);
    }
}
function patchHowlerPlay() {
    ...
    try {
        ...
        dbgWarn("audio-suppression | Howler patch failed", e);
    }
}
function patchHTMLAudioPlay() {
    if (PATCHED.html)
        return;
    try {
        const P = globalThis.HTMLMediaElement?.prototype;
        if (!P?.play)
            return;
        const _play = P.play;
        if (_play._bsrPatched) {
        ...
        dbgWarn("audio-suppression | HTMLAudio patch failed", e);
    }
}
export function installAudioPatches() {
...
export function registerAudioHooks() {
